...
#[test]
fn minimize_writes_output_dir() {
    let temp = tempdir().unwrap();
    let tape_dir = temp.path().join("tape");
    write_minimal_tape(&tape_dir);
    let out_dir = temp.path().join("minimized");
...
#[test]
fn minimize_refuses_overwrite_without_flag() {
    let temp = tempdir().unwrap();
    let tape_dir = temp.path().join("tape");
    write_minimal_tape(&tape_dir);
    let out_dir = temp.path().join("minimized");
    ...
    let mut second = Command::new(assert_cmd::cargo_bin!("incitape"));
    second
        .arg("minimize")
        .arg(&tape_dir)
        .arg("--out")
        .arg(&out_dir);
    second.assert().failure().code(2);
}
